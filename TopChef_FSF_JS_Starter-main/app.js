...
function searchCuisines(allDishes) {
    ...
    var userCuisine = customPrompt("What kind of cuisine do you want?", ["Italian", "Mexican", "French", "Irish", "Vegetarian", "Hungarian"]);
...
function searchIngredients(allDishes) {
    ...
    var userIngredient = customPrompt("What kind of ingredients are you looking for?", ["tomato", "cheese", "corn", "flour", "sugar", "beef", "cabbage", "chickpea", "parsley"])
    alert("Searching for dishes by ingredient...");

    let results = allDishes.filter(function (el){
        return el.ingredients.includes(userIngredient);
    });
      
    alert("Found all dishes matching the requested ingredient! Check the console for full output");
    console.log(results);
